...
namespace NCS.DSS.Contact.Models
{
    public class ContactDetails : IContactDetails
    {
        ...
        [StringLength(20)]
        [Example(Description = "UK phone number. Allows 3, 4 or 5 digit regional prefix, with 8, 7 or 6 digit phone number respectively, plus optional 3 or 4 digit extension number prefixed with a # symbol. " +
        ...
        [RegularExpression(@"^((\(?0\d{4}\)?\s?\d{3}\s?\d{3})|(\(?0\d{3}\)?\s?\d{3}\s?\d{4})|(\(?0\d{2}\)?\s?\d{4}\s?\d{4}))(\s?\#(\d{4}|\d{3}))?$")]
        public string HomeNumber { get; set; }

        [StringLength(20)]
        [Example(Description = "Alternative UK phone number. Allows 3, 4 or 5 digit regional prefix, with 8, 7 or 6 digit phone number respectively, plus optional 3 or 4 digit extension number prefixed with a # symbol. " +
                                "Also allows optional brackets surrounding the regional prefix and optional spaces between appropriate groups of numbers    e.g.   " +
                                "01222 555 555   or   (010) 55555555 #2222   or   0122 555 5555#222")]
        [RegularExpression(@"^((\(?0\d{4}\)?\s?\d{3}\s?\d{3})|(\(?0\d{3}\)?\s?\d{3}\s?\d{4})|(\(?0\d{2}\)?\s?\d{4}\s?\d{4}))(\s?\#(\d{4}|\d{3}))?$")]
